...
import { offgridContext, outageKit, optimizeB, OFFGRID_INPUTS } from '../model/offgrid.js';
...
const A = outageKit(ctx);
console.log(`A: worst week starts ${MON[A.start.month]} ${A.start.day}`);
...
writeFileSync(new URL('../data/offgrid.json', import.meta.url), JSON.stringify({ inputs: OFFGRID_INPUTS, grid, A, B }, round));
