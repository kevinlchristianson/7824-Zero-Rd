...
export const FIN_INPUTS = {
  ...
  strategy: {
    ...
    sellOn: '2026-12',
    prepayMortgage: false,   // owner: once the other debts are paid, rent and surplus flow into brokerage; the 2.875% mortgage just amortizes    // once every phase is paid, surplus prepays the rental mortgage
...
function simCore(inp) {
  ...
  for (let t = 1; t <= T; t++) {
    ...
    for (const key in inv) { const add = A[key + 'Add'] || 0; inv[key] = inv[key] * (1 + (A[key + 'Ret'] || 0) / 12) + add; contribIn += add; }
  ...
  const summary = {
    ...
    nwEnd: rows.at(-1).nw, nwNet: rows.at(-1).nw - paycheckIn - contribIn, contribIn, invEnd: rows.at(-1).invTotal,
  };
...
export function projectInputs(sim, cal) {
  ...
  Object.assign(I.accounts, { helocParts: Object.entries(r.helocParts).filter(([, v]) => v > 0.5).map(([id, v]) => [id, round2(v)]), renoLeft: round2(r.renoLeft), homeValue: Math.round(r.home), rothIra: round2(r.inv.rothIra), roth401k: round2(r.inv.roth401k), stable: round2(r.inv.stable), savings: round2(r.sav), brokerage: round2(r.brok), heloc: round2(r.heloc), mortgage: round2(r.mtg), truck: round2(r.truck), wf: round2(r.wf), usb: round2(r.usb), rentalValue: Math.round(r.value), rentalSold: r.sold });
