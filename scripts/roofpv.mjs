...
console.log(`Gable roofs at ${DEFAULTS.pitch * 12}/12 (${deg.toFixed(1)}°); ${S.panel.watts} W panels ${S.panel.longIn.toFixed(1)}" x ${S.panel.shortIn.toFixed(1)}", ${S.gapIn}" apart; ${S.edgeFt * 12}" clear of eaves and gable ends, ${S.ridgeFt * 12}" off ridges; spots losing over ${S.maxLoss * 100}% to shade left out`);
for (const sl of R.slopes) {
  if (!sl.south) { console.log(`  ${sl.name}: ridge runs north-south, no south slope`); continue; }
  console.log(`  ${sl.name}: slope ${sl.widthFt.toFixed(1)} x ${sl.lengthFt.toFixed(1)} ft, ${sl.orient} ${sl.rows} rows x ${sl.cols}: ${sl.panels.length} fit, ${sl.n} pay (${sl.kw.toFixed(1)} kW, ${n(sl.kwh)} kWh DC/yr), ${sl.panels.length - sl.n} too shaded`);
}
const T = R.totals;
